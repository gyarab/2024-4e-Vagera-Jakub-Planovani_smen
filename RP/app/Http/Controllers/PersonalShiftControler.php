...
class PersonalShiftControler extends Controller
{
    ...
    function today_offer()
    {
       ...
       // $today_offer = DB::select("SELECT * FROM shift_active_data, shift_model, object_model, shift_offer, shift_assignment WHERE shift_offer.date='$td' AND shift_assignment.id='$id' AND shift_offer.id_shift = shift_model.id_shift AND shift_offer.id_shift = shift_assignment.id_shift AND shift_model.id_object = object_model.id_object AND shift_active_data.id_shift=shift_offer.id_shift AND shift_active_data.saved_at='$td' ORDER BY shift_active_data.saved_from");
    ...
    public function number_planned1(){
        ...
        //$planned2 = DB::select("SELECT COUNT(*) FROM shift_stored_data WHERE shift_stored_data.saved_at LIKE '$date%' AND shift_stored_data.id='$id' AND shift_stored_data.id NOT IN (SELECT id FROM shift_active_data)");
        //$countofopps = count( $plannes);
        $supress = $planned1[0];
        //$supress2 = $planned2[0];
        //$supress = $supress1 + $supress2;
        //$countofopps = count($planned);
        return $supress;
    }
    public function number_planned2(){
        $id = Auth::id();
        $date = date('Y-m');
        //$planned1 = DB::select("SELECT COUNT(*) FROM shift_active_data WHERE saved_date LIKE '$date%' AND id_user='$id' ");
        $planned2 = DB::select("SELECT COUNT(*) FROM shift_planned_data WHERE shift_planned_data.saved_at LIKE '$date%' AND shift_planned_data.id='$id' AND shift_planned_data.id_planned NOT IN (SELECT id_active FROM shift_active_data)");
        //$countofopps = count( $plannes);
        $supress = $planned2[0];
        //$supress2 = $planned2[0];
        //$supress = $supress1 + $supress2;
    ...
    function number_worked(){
        $id = Auth::id();
        $date = date('Y-m');
        $worked = DB::select("SELECT COUNT(*) FROM shift_planned_data, attendance WHERE shift_planned_data.saved_at LIKE '$date%' AND shift_planned_data.id='$id' AND shift_planned_data.id_planned=attendance.id_planned");
        $supress = $worked[0];
        return  $supress;
    }
    /*LIKE '%a';*/
